%   The object-level fusion version of the reliable and adaptive spatiotemporal data fusion (OL-RASDF) method
...
ww=1;   % window size in Local unmixing step
...
segfile = 'E:\ThirdPaper\PY2\select\segL1.tif';
seg = imread(segfile);
snum = max(max(seg));

outputname = 'E:\RASDF_neg47';
...
for class=min_class:max_class
    ...
    for b=1:bands
        ...
        tmp=Estimate_endmember_change_each_band(CR_fraction_T1 ,CRT2Cb ,CRT1Cb ,xL,yL,class,1,mm,nn,mmm,nnn,min_allowb,max_allowb,pp);
